...
import { layoutTitle } from "../functions/lib/og-title.mjs";
...
/* The image URL must change whenever its pixels can change. Hash the renderer
   and the two fonts it uses automatically; each entry adds title/date below. */
...
function prepareEntries(entries) {
  ...
  for (const entry of entries) {
    ...
    entry.imageUrl = `${SITE}/api/og?slug=${entry.slug}&v=${entry.imageVersion}`;
  }
}
...
function renderEntryPages(entries) {
  ...
  for (const entry of entries) {
    ...
    const jsonLd = {
      ...
      "image": { "@type": "ImageObject", "url": entry.imageUrl, "width": 1200, "height": 630 },
    };
    const page = fillTemplate(template, {
      ...
      IMAGE_URL: escapeHtml(entry.imageUrl),
